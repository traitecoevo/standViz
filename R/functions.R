...
#' @param crown_shape Shape of crown. See \code{make_shape} for allowable values.
...
#' @export
make_stand_dataframe <- function(trees, crown_shape = "yokozawa", eta = 5,
...
#' Creates a 3D shape matrix suitable for passing to \code{rgl::plot.triangles}, and used
#' by \code{plot3d_shape}) This object has height 1, width 1, and is located at (0,0,0). Use
#' the function \code{resize_shape} to scale and move to a different set of coordinates
#' @title Creates a 3D shape matrix suitable
...
#' @export
make_shape <- function(shape = c("cone", "ellipsoid", "halfellipsoid",
...
#' @param trees A dataframe of values for the stand. Required values are defined in code{make_stand_dataframe}
#' @param scaling a vector of length two, giving scaling of heights and widths in the plot
#' @param ... Additional arguments defining crown, to pass through to \code{make_shape}
#' @export
make_stand <- function(trees, scaling = c(1, 1), ...) {

  trees <- make_stand_dataframe(trees)
  ...
  for (s in unique(trees[["species"]]) ) {
    ...
    crown <- make_shape(shape = as.character(d[["crown_shape"]][1]),
                        d[["eta"]][1], ...)

    stem <- make_shape(shape = "cone", nz = 2, nalpha = 5)
...
#' @param crown A matrix with columns [X,Y,Z] describing shape of the crown
#' @param stem A matrix with columns [X,Y,Z] describing shape of the stem
#' @param crown_shape One of 'cone', 'elipsoid', 'ellipsoid', 'round', 'halfellipsoid', 'paraboloid', 'cylinder', 'yokozawa'
...
#' @param scaling a vector of length two, giving scaling of heights and widths in the plot
#' @param ... Additional arguments defining crown, to pass through to \code{make_shape}
#' @return A list with elements  "crown", "crown_color", "stem", "stem_color" defining the
...
make_tree <- function(x = 0, y = 0, height = 1, height_crown_base = 0,
                      ...
                      crown_shape = c("cone", "elipsoid", "ellipsoid",
                          ...
                          "yokozawa"),
                      eta = 13, crown_color = "forestgreen", stem = NULL,
                      stem_color = "brown",
...
#' @param shape Matrix with columns [X,Y,Z]
#' @param ... Additional arguments passed through to \code{rgl::rgl.triangles}
plot_shapes <- function(shape, ...) {
  rgl::rgl.triangles(shape[, 1], shape[, 2], shape[, 3], ...)
}
...
#' @param stand A list containing trees, as generated \code{make_stand}
#' @param add Add to existing plot?
#' @param size A vector of dimensions for the plot, as specified in \code{new_rgl}
#' @param ... Additional arguments passed through to \code{rgl::rgl.triangles}
#' @export
plot_stand <- function(stand, add = FALSE, size = c(1, 1, 500, 500), ...) {

  if (!add) {
    new_rgl(size = size)
  }

  crown_colors <- sapply(stand, function(tree) tree[["crown_color"]])
  stem_colors <- sapply(stand, function(tree) tree[["stem_color"]])

  get_item <- function(n, stand, v = "crown") stand[[n]][[v]]

  for (i in unique(crown_colors) ) {
    shapes <- do.call(rbind, lapply(which(i == crown_colors), get_item,
                                        stand = stand) )
    plot_shapes(shapes, col = i, ...)
  }

  for (i in unique(stem_colors) ) {
    shapes <- do.call(rbind, lapply(which(i == stem_colors), get_item,
                                        stand = stand, v = "stem") )
    plot_shapes(shapes, col = i, ...)
  }
}
...
plot_tree <- function(tree, ...) {
  plot_shapes(tree[["crown"]], col = tree[["crown_color"]], ...)
  plot_shapes(tree[["stem"]], col = tree[["stem_color"]], ...)
}
...
resize_shape <- function(shape, height = 1, width = 1, x = 0, y = 0, z = 0) {
  shape[, 1] <- x + 0.5 * width * (shape[, 1] / max(shape[, 1]) )
  shape[, 2] <- y + 0.5 * width * (shape[, 2] / max(shape[, 2]) )
  shape[, 3] <- z + height * (shape[, 3] / max(shape[, 3]) )
  shape
}
